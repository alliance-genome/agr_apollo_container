...
if [[ "$?" == "1" ]]; then
  ...
  su postgres -c "PGPASSWORD=$WEBAPOLLO_DB_PASSWORD  psql $WEBAPOLLO_HOST_FLAG -U $WEBAPOLLO_DB_USERNAME -h $WEBAPOLLO_DB_HOST $WEBAPOLLO_DB_NAME -f /agr-apollo-load.sql ;"
fi
